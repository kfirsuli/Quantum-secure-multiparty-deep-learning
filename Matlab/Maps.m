...
G = 1+logspace(-2.1,log10(2.5),512);
mu = logspace(-2, log10(50), 512);
...
epsilon = (2 * G - 2) ./ G  /n;
w_leak = 0.5*Leak(2*mu+1,1,epsilon); %0.5 accounf for real or image elements
%d_leak = CCRB1(G, n, 1/sqrt(n));
d_leak = 0.5*log2(1+2-2./G);
g = 2-2./G;
d_leak = 402/392*0.5*(n*log2(1+g/n)-log2(1+g));
d_leak = (784+10)/784*(n*log2(1+g/n)-log2(1+g));
g = 2-2./G;
c = g.^2./(1+g);
M=784+10;
d_leak = 0.5*log2(1+4*(M./n).*(G-1).^2./(G.*(3*G-2))); % 0.5 counts for real and image in addition to the 0.5
% d_leak = 0.5*log2(1+(M./n).*c);
...
contourLevels = [10 20 30 40 50 60 70 80 90 95 97];
%contourLevels = [20 40 60 80 95];
...
%xlabel('$Weights Leakage \vec{\mathrm{w}}$ (bits)', 'Interpreter', 'latex', 'FontSize', 18, 'FontName', 'Cambria Math')
%ylabel('$Data \vec{\mathrm{x}}$ Leakage (bits)', 'Interpreter', 'latex', 'FontSize', 18, 'FontName', 'Cambria Math')

xlabel('$\textrm{Weights Leakage } \mathcal{I}_{w}$ (bits)', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Cambria Math');
ylabel('$\textrm{Data Leakage } \mathcal{I}_{x}$ (bits)', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Cambria Math');

% xlabel('Weights Leakage (bits)')
% ylabel('Data Leakage (bits)')
set(gca, 'FontSize', 14)
% hcb = colorbar;
% ylabel(hcb, 'Classification Accuracy (%)', 'Interpreter', 'latex', 'FontSize', 18)
hcb=colorbar;
ylabel(hcb,[{'Classification Accuracy (%)'}],'FontSize',14)
ylabel(hcb,'Classification Accuracy $\mathrm{Acc}(\mathcal{I}_W,\mathcal{I}_x)$ (\%)', ...
    'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Cambria Math');
ylabel(hcb,'Classification Accuracy (\%)');
%set(hcb, 'FontName', 'Times New Roman')
% ylabel(hcb, '$f(\vec{x},\vec{w})$ Classification Accuracy (%)', 'Interpreter', 'latex', 'FontSize', 14)
% ylabel(hcb, '$f(\vec{x},\vec{w})$ Classification Accuracy (\%)', 'Interpreter', 'latex', 'FontSize', 14)
set(hcb, 'FontSize', 14)
